        ...
        <thead class="bg-gray-50">
          <tr>
            ...
            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nº
              Proposta</th>
            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 tracking-wider relative dropdown-container">
              ...
              <div class="dropdown-menu hidden absolute z-50 mt-2 w-72 bg-white border border-gray-200 rounded-lg shadow-2xl p-3 normal-case font-normal text-gray-700 left-0 top-full">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          @forelse ($orcamentos as $orcamento)
            <tr>
              ...
              <td class="max-w-[226px] truncate py-4 whitespace-nowrap text-sm text-gray-500" title="{{ $orcamento->cliente->nome ?? 'N/A' }}">
                {{ $orcamento->cliente->nome ?? 'N/A' }}</td>
              <td class="max-w-[265px] truncate px-6 py-4 whitespace-nowrap text-sm text-gray-500" title="{{ $orcamento->escopo ?: 'Não definido' }}">
                {{ $orcamento->escopo ?: 'Não definido' }}</td>
                ...
                <div class="flex flex-col md:flex-row gap-6 items-start">
                  <div class="flex flex-col gap-2 md:w-1/3 text-gray-500">
                    <p><strong>Valor: </strong>R$
                      {{ number_format($orcamento->valor, 2, ',', '.') }}</p>
                    <p><strong>Data de Solicitação:</strong>
                      {{ $orcamento->data_solicitacao ? \Carbon\Carbon::parse($orcamento->data_solicitacao)->format('d/m/Y') : 'Não definida' }}
                    </p>
                    <p><strong>Data de Envio:</strong>
                      {{ $orcamento->data_envio ? \Carbon\Carbon::parse($orcamento->data_envio)->format('d/m/Y') : 'Não definida' }}
                    </p>
                    <p><strong>Data de Aprovação:</strong>
                      {{ $orcamento->data_aprovacao ? \Carbon\Carbon::parse($orcamento->data_aprovacao)->format('d/m/Y') : 'Não definida' }}
                    </p>
                    ...
                    <p class="whitespace-pre-line {{ $orcamento->comentario ? 'text-blue-800 font-bold' : 'text-gray-500 italic' }}">
                      {{ $orcamento->comentario ?: 'Nenhum comentário adicionado.' }}
                    </p>
                  ...
                  <div class="flex flex-col gap-2 md:w-1/3">
                            ...
                            <div class="flex items-center overflow-hidden">
                              ...
                              <div class="truncate">
                                ...
                                <p class="text-xs text-gray-400">
                                  {{ $anexo->created_at->format('d/m/Y H:i') }}</p>
                              </div>
                            </div>
                    ...
                    @else
                      <p class="text-sm text-gray-500 italic">Nenhum anexo encontrado para este
                        orçamento.</p>
                    @endif
                  </div>
                  <div class="flex flex-col gap-2 md:w-1/3">
                    <div class="bg-blue-50 border border-blue-200 rounded-md p-3 h-full">
                      <div class="flex justify-between items-center mb-3 border-b border-blue-200 pb-2">
                        ...
                        <button type="button" onclick='openGeneralHistoryModal(@json($orcamento->history), @json($labelsOrcamento))' class="text-xs bg-white border border-blue-300 text-blue-700 hover:bg-blue-600 hover:text-white px-2 py-1 rounded transition shadow-sm">
                          Ver Completo
                        </button>
                      </div>
                      ...
                      @if ($orcamento->last_user_id && (!$orcamento->history || $orcamento->history->count() == 0))
                        <div class="mt-2 pt-2 border-t border-blue-100 text-xs text-gray-500 text-center">
                          Última ação por:
                          <strong>{{ $orcamento->editor->name ?? 'Sistema' }}</strong><br>
